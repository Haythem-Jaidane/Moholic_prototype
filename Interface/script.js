...
new Vue({
  ...
  computed: {
    ...
    hasArtistProfile(){
      return(this.CurrentPage=="ArtistProfile") ? true:false;
    },

    hasMarketplace(){
      return(this.CurrentPage=="market") ? true:false;
    },
    
    hasPlaylist(){  
      return(this.CurrentPage=="playlist") ? true:false;
    },

    hasMainPage(){
      return(this.CurrentPage=="main") ? true:false;
    },
  ...
  methods: {
    //
    playListVerfication(owner_id){
      return(owner_id==this.connectedUserId) ? true:false;
    },

    // *******************  API data JSON file manipulation methods *******************

    // Add Data to json file
    addDataJSON(file,data_array){
      axios.post(file,data_array);
    },
    ...
    addUser(id,name){
      ...
      console.log(this.Users);
      this.addDataJSON("./json/Users.json",this.Users);*/
    ...
    /*getMarketpalace(){
      let endpoint = './json/Market.json';
      let emsg = [ 'There was a problem trying to load the MarketPlace.' ];

      axios.get( endpoint ).then( res => {
        if ( !res || !res.data || !res.data.Market ) {
          emsg.push( 'The API response did not have any channels data available at this time.' );
          emsg.push( 'Status: Channels API Error.' );
          return this.setError( 'Market', emsg.join( ' ' ) );
        }
        for ( let m of res.data.Market ) {
          m.title = m.title;
          m.image=m.image;
          m.artist=m.artist;
          m.owner=m.owner;
        ...
        this.Market = res.data.Market.slice();
        this.setError( 'init', '' );
        this.setError( 'Market', '' );
      })
    },*/
 
    //
    getSongs() {
      let endpoint = './json/SongCopies.json';
      let emsg = [ 'There was a problem trying to load the list of available Songs Copies' ];

      axios.get( endpoint ).then( res => {
        if ( !res || !res.data || !res.data.SongCopies ) {
          emsg.push( 'The API response did not have any channels data available at this time.' );
          emsg.push( 'Status: Channels API Error.' );
          return this.setError( 'SongCopies', emsg.join( ' ' ) );
        }
        for ( let s of res.data.SongCopies ) {
          // filter and sanitize list of channels
          s.id = s.id;
          s.title=s.title;
          s.image=s.image;
          s.mp3=s.mp3;
          s.mp3_demo=s.mp3_demo;
          s.artist=s.artist;
          s.price=s.price;
          s.owner=s.owner;
        ...
        this.SongCopies = res.data.SongCopies.slice();
        this.setError( 'init', '' );
        this.setError( 'SongCopies', '' );
      })
    },

    // get user data from api
    getUser(){
      let endpoint = './json/Users.json';
      let emsg = [ 'There was a problem trying to load the MarketPlace.' ];
    ...
    // get user data from api
    getOriginal(){
      let endpoint = './json/SongsOriginal.json';
      let emsg = [ 'There was a problem trying to load the MarketPlace.' ];

      axios.get( endpoint ).then( res => {
        if ( !res || !res.data || !res.data.SongsOriginal ) {
          emsg.push( 'The API response did not have any channels data available at this time.' );
          emsg.push( 'Status: Channels API Error.' );
          return this.setError( 'Market', emsg.join( ' ' ) );
        }
        for ( let s of res.data.SongsOriginal ) {
          s.id = s.id;
          s.title=s.title;
          s.image=s.image;
          s.mp3=s.mp3;
          s.mp3_demo=s.mp3_demo;
          s.artist=s.artist;
          s.artist_id=s.artist_id;
          s.price=s.price;
        }
        this.SongsOriginal = res.data.SongsOriginal.slice();
        this.setError( 'init', '' );
        this.setError( 'Market', '' );
      })
    },
    ...
    // select an artist to view his profil
    selectArtist(Artist){
      this.Artistprofil=Artist;
      this.CurrentPage="ArtistProfile";
    },
